...
This variant uses classic arcade-style physics: a constant downward
gravity acceleration applied every frame, and an instantaneous upward
velocity impulse on jump (instead of the cosine-eased timed climb used
in flappybird.py).
"""
...
class Bird(pygame.sprite.Sprite):
    ...
    (set its velocity to a fixed upward impulse), otherwise gravity
    constantly accelerates it downward.  It must pass through the space
    in between pipes (for every pipe passed, one point is scored); if it
    crashes into a pipe, the game ends.

    Attributes:
    x: The bird's X coordinate.
    y: The bird's Y coordinate.
    velocity: The bird's current vertical velocity, in pixels per
        millisecond.  Negative values mean upward movement.

    Constants:
    WIDTH: The width, in pixels, of the bird's image.
    HEIGHT: The height, in pixels, of the bird's image.
    GRAVITY: The constant downward acceleration, in pixels per
        millisecond squared, applied to the bird every frame.
    JUMP_STRENGTH: The velocity, in pixels per millisecond, the bird's
        velocity is set to (overwriting any existing velocity) when the
        player presses the jump key.  Negative because upward is
        negative y.
    MAX_FALL_SPEED: The maximum downward velocity the bird can reach.
    """
    ...
    def __init__(self, x, y, velocity, images):
        ...
        velocity: The bird's initial vertical velocity, in pixels per
            millisecond.
    ...
    def jump(self):
        ...
        This overwrites the bird's current velocity with
        Bird.JUMP_STRENGTH, rather than adding to it, so the jump always
        feels the same regardless of whether the bird was rising or
        falling when the key was pressed.
        """
        self.velocity = Bird.JUMP_STRENGTH

    def update(self, delta_frames=1):
        """Update the bird's velocity and position.

        Gravity is applied as a constant acceleration every frame,
        increasing the bird's (downward) velocity, which is then used to
        move the bird.  This produces a natural parabolic arc after a
        jump.
        ...
        self.velocity = min(self.velocity + Bird.GRAVITY * dt,
                             Bird.MAX_FALL_SPEED)
...
def main():
    ...
    pygame.display.set_caption('Pygame Flappy Bird (velocity/gravity physics)')
    ...
    bird = Bird(50, int(WIN_HEIGHT/2 - Bird.HEIGHT/2), 0.0,
...
if __name__ == '__main__':
    # If this module had been imported, __name__ would be 'flappybird_velocity'.
