...
echo "==> Switching all three minigames to the Pi camera (rpicam) ..."
...
ssh "${PI_HOST}" '
  cd ~/12FA_SoapDispensor_Game/Minigames &&
...
echo ""
echo "==> Done. On the Pi (conda env 'game'), from ~/12FA_SoapDispensor_Game/Minigames:"
echo "      Guessing game : python -m Guessing_Game.main"
echo "      RPS           : cd RPS-Game && python RPS_vs_Computer.py"
echo "      Rubik's       : cd rubiks-vision && python main.py"
